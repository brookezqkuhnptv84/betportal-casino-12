...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-a02fc00b";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-a02fc00b">'+'<span class="tile-face-a02fc00b tile-front-a02fc00b"></span>'+'<span class="tile-face-a02fc00b tile-back-a02fc00b"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a02fc00b")){return;}
var back=tile.querySelector(".tile-back-a02fc00b");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-a02fc00b");if(isMine){tile.classList.add("boom-a02fc00b");}
if(dim){tile.classList.add("dimmed-a02fc00b");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-a02fc00b")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-a02fc00b");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a02fc00b")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-a02fc00b");void panelEl.offsetWidth;panelEl.classList.add("jolt-a02fc00b");}
endRound();}
